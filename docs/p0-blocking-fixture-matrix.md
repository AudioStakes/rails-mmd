...
- Post-output blocking fixtures must assert diagnostics are published and the
  affected `.mmd` and render-plan artifacts are omitted.
...
| code | phase | trigger fixture | exit | stderr | artifacts |
|---|---|---|---:|---|---|
| `CONFIG_NOT_FOUND` | config | no `rails_mmd.yml` and no readable `--config` path | 2 | yes | none |
...
| `MULTI_DB_UNSUPPORTED` | schema probe | selected models require multiple connection contexts | 2 | no | diagnostics only |
| `MODEL_TABLE_MISSING` | schema probe | selected model table is missing or inaccessible | 2 | no | diagnostics only |
| `MODEL_PRIMARY_KEY_UNSUPPORTED` | schema probe | primary key is missing, composite, or unsupported | 2 | no | diagnostics only |
| `SAFE_TOKEN_COLLISION` | tokenization | suffix exhaustion leaves an unresolved safe-token collision | 3 | no | diagnostics only |
| `MERMAID_SERIALIZATION_FAILED` | serialization | render plan cannot be serialized to valid Mermaid text | 3 | no | diagnostics only |
| `OUTPUT_WRITE_FAILED` | publish | atomic write, rename, cleanup, or rollback fails | 4 | yes when no diagnostic file can be trusted | previous artifacts preserved or diagnostics only |
| `INTERNAL_ERROR` | internal | unexpected exception escapes a P0 stage | 99 | yes before output, otherwise no | none or diagnostics only |
